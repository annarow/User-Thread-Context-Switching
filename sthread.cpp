...
/*
430 Program 2 Part 1 Threads
 for methods sthread_yield and capture
2/4/2022
By: Anna Rowena Waldron
 */

/*
 * Method that is called many times to save the enviroment of the current thread and its memory.
 * Makes a copy and pushes onto the queue.
 */
#define capture()                   \
{                                   \
    ...
    if(cur_tcb->stack_ == NULL){    \
        cur_tcb->stack_ = malloc(sizeof(TCB));                    \
        }\
...
/*
 * Method that waits for an alarm interrupt to yield CPU to another thread.
 * Calls the capture method to keep track of contents of current thread. Uses jumps
 * to go to the scheduler and return to finish executing current thread after yielding.
 * Pushes to the queue the thread to be run.
 */
